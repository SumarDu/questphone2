...
data class AppInfo(val originalLabel: String, val label: String, val packageName: String, val icon: Drawable)
            ...
            val apps = profiles.flatMap { userHandle ->
                launcherApps.getActivityList(null, userHandle).mapNotNull { activityInfo ->
                    try {
                        ...
                        AppInfo(label, label, appInfo.packageName, icon)
                    } catch (e: Exception) {
                        null
                    }
                }
